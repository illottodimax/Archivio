...
def carica_archivio_ruota(nome_ruota_str):
    ...
    if nome_ruota_str in ARCHIVI_CARICATI:
        gui_output(f"Utilizzo archivio {nome_ruota_str} già in memoria.") # Messaggio leggermente modificato
    ...
    if os.path.exists(file_locale_path):
        gui_output(f"Trovato archivio locale per {nome_ruota_str}. Carico...") # Messaggio modificato
        try:
        ...
        try:
            ...
            if archivio_crono_dl:
                ...
                try:
                    ...
                    gui_output(f"Archivio salvato/aggiornato in '{os.path.basename(file_locale_path)}'.") # Mostra solo nome file
                except IOError as e:
...
# --- Funzione di Analisi (Quindicine) ---
def analizza_ruota_lotto(nome_ruota_scelta, archivio_ruota_crono, num_estrazioni_da_analizzare):
    global analysis_running
    start_time_analysis = time.time()
    try:
        gui_output(f"\nInizio analisi QUINDICINE per Ruota: {nome_ruota_scelta} ({num_estrazioni_da_analizzare} estrazioni)", tags=(TAG_BOLD,), clear=True)
        ...
        for i in range(6):
            ...
            for j in range(1, 16):
                num_reale = i * 15 + j
                if num_reale <= 90:
                    tens_digit = num_reale // 10
                    units_digit = num_reale % 10
                    part_tens += str(tens_digit)
                    part_units += str(units_digit)
                else: pass
            header_tens_parts.append(part_tens)
            header_units_parts.append(part_units)
        gui_output(f"{spazio_iniziale}{' '.join(header_tens_parts)}")
        gui_output(f"{spazio_iniziale}{' '.join(header_units_parts)}")
        gui_output("")
        ...
        for idx_corr in range(indice_inizio_ciclo, indice_fine_ciclo + 1):
            ...
            for idx_spia_0based in range(5):
                ...
                quindicina_spia = (numero_spia - 1) // 15
                        ...
                        for j in range(1, ESTRAZIONI_SUCCESSIVE_VBS + 1):
                            ...
                            if idx_succ <= idx_corr:
                                for pos_succ_0based in range(5):
                                    ...
                                    if 1 <= num_succ <= 90 and (num_succ - 1) // 15 == quindicina_spia:
            ...
            if idx_corr > indice_inizio_ciclo:
                for k in range(5):
                    ...
                    if 1 <= num_c <= 90 and 1 <= num_p <= 90 and (num_c - 1) // 15 == (num_p - 1) // 15:
                        colora_rosso[k] = True

            segmenti_riga = []; data_str = data_estrazione_da_archivio(archivio_ruota_crono, idx_corr)
            segmenti_riga.append({'text': f"{data_str:<10s} ", 'tags': (TAG_BOLD,)})

            for i in range(6):
                for j in range(1, 16):
                    num_reale = i * 15 + j
                    if num_reale <= 90:
                        conteggio = conteggio_numeri[num_reale]
        ...
        gui_output("-" * (len(nome_ruota_scelta) + 30))
        gui_output(f"Analisi QUINDICINE per Ruota: {nome_ruota_scelta} completata.", tags=(TAG_BOLD,))
...
def select_folder():
    """Apre la finestra di dialogo per selezionare la cartella degli archivi."""
    ...
    if directory:
        ...
        # Mostra conferma nella status bar invece che nel log principale per meno "rumore"
        gui_output(f"Cartella archivi impostata.", status=True) # Messaggio generico
...
def start_analysis_thread():
    ...
    if not os.path.isdir(selected_downloads_dir):
        messagebox.showerror("Errore", f"La cartella selezionata non esiste! ({selected_downloads_dir})") # Mostra percorso nell'errore
        return
    ...
    status_label.config(text=f"Avvio analisi QUINDICINE per {nome_ruota_selezionata}...", foreground='black')
    ...
    def analysis_task():
        ...
        try:
            ...
            if archivio_crono:
                 gui_output(f"Archivio caricato ({len(archivio_crono)} estr.). Avvio calcoli QUINDICINE...", status=True)
...
root = tk.Tk()
root.title("Analisi Statistica Lotto (Quindicine - VB Logic Emulation)")
...
# --- WIDGET RIMOSSI ---
# ttk.Label(folder_frame, text="Cartella Archivi:").pack(side=tk.LEFT, padx=(0, 5))
# folder_path_var = tk.StringVar(root) # <<< RIMOSSO
# folder_path_var.set(selected_downloads_dir) # <<< RIMOSSO
# folder_entry = ttk.Entry(folder_frame, textvariable=folder_path_var, width=90, state='readonly') # <<< RIMOSSO
# folder_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5) # <<< RIMOSSO
# --- FINE WIDGET RIMOSSI ---

# Pulsante per scegliere la cartella (ora occupa lo spazio disponibile o è a sinistra)
folder_button = ttk.Button(folder_frame, text="Scegli Cartella Archivi...", command=select_folder) # Testo leggermente modificato
# Pack semplice a sinistra, oppure potresti provare fill=tk.X per farlo espandere
...
output_text = scrolledtext.ScrolledText(output_frame, wrap=tk.NONE, state=tk.DISABLED, height=25, width=140)
...
if __name__ == "__main__":
    ...
    gui_output("   Analisi Statistica Lotto (QUINDICINE - VB Logic Emulation)    ", tags=(TAG_BOLD,))
    gui_output("===================================================================", tags=(TAG_BOLD,))
    # gui_output(f"Cartella archivi iniziale: '{selected_downloads_dir}'") # <<< RIMOSSO MESSAGGIO INIZIALE PERCORSO
    gui_output("Seleziona ruota, numero ultime estrazioni (default 30) e clicca 'Analizza'.")
    gui_output("Usa 'Scegli Cartella Archivi...' per selezionare la cartella con i file .txt.") # Messaggio aggiornato

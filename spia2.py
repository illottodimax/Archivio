...
# MODIFICATO per includere pulsante "Salva su File..."
def mostra_popup_testo_semplice(titolo, contenuto_testo):
    ...
    ttk.Button(button_frame_popup, text="Salva su File...", command=_salva_popup_content).pack(side=tk.LEFT, padx=5)
...
def mostra_popup_risultati_spia(info_ricerca, risultati_analisi):
                    ...
                    if top_pres is not None and not top_pres.empty:
                        for item, pres_val in top_pres.items():
                            ...
                            if tipo_esito == 'estratto' and 'posizionale_presenza' in dati_esito and item_f in dati_esito['posizionale_presenza']:
                                pos_data = dati_esito['posizionale_presenza'][item_f]; pos_str_list = []
                    ...
                    if top_freq is not None and not top_freq.empty:
                        for item, freq_val in top_freq.items():
                            ...
                            if tipo_esito == 'estratto' and 'posizionale_frequenza' in dati_esito and item_f in dati_esito['posizionale_frequenza']:
                                pos_data = dati_esito['posizionale_frequenza'][item_f]; pos_str_list = []
                                for pos_num in sorted(pos_data.keys()):
        ...
        for tipo_esito_comb in ['estratto', 'ambo', 'terno']:
            ...
            if dati_tipo_comb_dett:
                ...
                for i, stat_item in enumerate(dati_tipo_comb_dett):
                    item_str_c = stat_item["item"]; score_c = stat_item["punteggio"]; pres_avg_c = stat_item["presenza_media_perc"]; freq_tot_c = stat_item["frequenza_totale"]
                    popup_content_list.append(f"    {i+1}. {item_str_c}: Punt={score_c:.2f} (PresAvg:{pres_avg_c:.1f}%, FreqTot:{freq_tot_c})")
    ...
    if migliori_ambi_globali_info:
        ...
        for i, ambo_info in enumerate(migliori_ambi_globali_info):
            popup_content_list.append(f"  {i+1}. Ambo {ambo_info['ambo']}: Coperti {ambo_info['coperti']} su {ambo_info['totali']} eventi ({ambo_info['percentuale']:.1f}%)")
    ...
    if combinazione_ottimale_info:
        ...
        popup_content_list.append(f"  I seguenti {len(combinazione_ottimale_info['ambi'])} ambo/i:")
        for ambo_s in combinazione_ottimale_info['ambi']: popup_content_list.append(f"    - {ambo_s}")
        ...
        popup_content_list.append(f"  Considerando i Top {len(migliore_parziale_info['ambi'])} ambi:")
        for ambo_s in migliore_parziale_info['ambi']: popup_content_list.append(f"    - {ambo_s}")
...
def cerca_numeri(modalita="successivi"):
    ...
    if modalita == "successivi":
        ...
        for nome_rv_loop in nomi_rv:
            df_ver_full = df_cache_ver.get(nome_rv_loop)
            if df_ver_full is None: df_ver_full=carica_dati(file_ruote.get(nome_rv_loop),start_date=start_ts, end_date=None); df_cache_ver[nome_rv_loop]=df_ver_full
            if df_ver_full is None or df_ver_full.empty: messaggi_out.append(f"[{nome_rv_loop}] No dati Ver."); continue
            res_ver,err_ver=analizza_ruota_verifica(df_ver_full,date_trig_ord,n_estr,nome_rv_loop)
            if err_ver: messaggi_out.append(f"[{nome_rv_loop}] Errore: {err_ver}"); continue
            if res_ver:
                ris_graf_loc.append((nome_rv_loop, spia_display_str, res_ver)); num_rv_ok+=1
                    ...
                    if res_s_out:
                        ...
                        if not res_s_out['presenza']['top'].empty:
                            for i,(item,pres) in enumerate(res_s_out['presenza']['top'].items()):
                                item_str_out = item; perc_p_out=res_s_out['presenza']['percentuali'].get(item,0.0); freq_p_out=res_s_out['presenza']['frequenze'].get(item,0)
        ...
        if ris_graf_loc and num_rv_ok > 0:
            ...
            for tipo_comb in ['estratto','ambo','terno']:
                messaggi_out.append(f"\n--- Combinati: {tipo_comb.upper()} Successivi (per Punteggio) ---"); comb_pres_dict,comb_freq_dict,has_data_comb={},{},False
                for _,_,res_comb_loop in ris_graf_loc:
                    if res_comb_loop and res_comb_loop.get(tipo_comb):
                        has_data_comb=True; current_presenze = res_comb_loop[tipo_comb].get('full_presenze', pd.Series(dtype=int)); current_frequenze = res_comb_loop[tipo_comb].get('full_frequenze', pd.Series(dtype=int))
                        for item_c,count_c in current_presenze.items(): comb_pres_dict[item_c]=comb_pres_dict.get(item_c,0)+count_c
                        for item_c,count_c in current_frequenze.items(): comb_freq_dict[item_c]=comb_freq_dict.get(item_c,0)+count_c
                ...
                sortable_index_list_comb = sorted(list(all_items_idx_comb_orig_keys), key=get_sortable_key_comb); ordered_index_comb = pd.Index(sortable_index_list_comb)
                ...
                punt_comb_s=((peso_pres*comb_perc_pres_s)+(peso_freq*comb_freq_norm_s)).round(2).sort_values(ascending=False); top_punt_comb=punt_comb_s.head(10)
                if not top_punt_comb.empty:
                    top_comb_ver[tipo_comb]=top_punt_comb.index.tolist()[:10]; stat_dett_comb = []
                    for item_idx_loop, score_val_loop in top_punt_comb.items(): stat_dett_comb.append({"item": format_ambo_terno(item_idx_loop), "punteggio": score_val_loop, "presenza_media_perc": comb_perc_pres_s.get(item_idx_loop, 0.0), "frequenza_totale": comb_freq_s.get(item_idx_loop, 0)})
                ...
                if not top_punt_comb.empty:
                    for i,(item_comb_idx_loop,score_comb_loop) in enumerate(top_punt_comb.items()): messaggi_out.append(f"    {i+1}. {format_ambo_terno(item_comb_idx_loop)}: Punt={score_comb_loop:.2f} (PresAvg:{comb_perc_pres_s.get(item_comb_idx_loop,0.0):.1f}%, FreqTot:{comb_freq_s.get(item_comb_idx_loop,0)})\n")
        ...
        if ris_graf_loc and num_rv_ok > 0 and n_trig_tot > 0:
            ...
            for data_trigger_evento in date_trig_ord:
                ...
                for nome_rv_check_glob in nomi_rv:
                    df_ver_loop = df_cache_ver.get(nome_rv_check_glob)
            ...
            if not conteggio_copertura_globale: messaggi_out.append("    Nessun ambo ha coperto eventi spia (considerando tutte le ruote di verifica)."); info_curr['migliori_ambi_copertura_globale'] = []
            else:
                ...
                if num_top_ambi_globali_display > 0 :
                    for i in range(num_top_ambi_globali_display):
                        ambo_glob_tuple, count_glob = migliori_ambi_globali_raw[i]; ambo_glob_str = format_ambo_terno(ambo_glob_tuple); perc_glob = (count_glob / n_trig_tot * 100) if n_trig_tot > 0 else 0
                        messaggi_out.append(f"    {i+1}. Ambo {ambo_glob_str}: Coperti {count_glob} su {n_trig_tot} eventi spia ({perc_glob:.1f}%)")
                        info_curr['migliori_ambi_copertura_globale'].append({"ambo": ambo_glob_str, "coperti": count_glob, "totali": n_trig_tot, "percentuale": perc_glob})
                    info_curr['combinazione_ottimale_copertura_100'] = None; info_curr['migliore_combinazione_parziale'] = None
                    if len(migliori_ambi_globali_raw) > 0 and n_trig_tot > 0: 
                        messaggi_out.append(f"\n  RICERCA COMBINAZIONE AMBI PER COPERTURA TOTALE ({n_trig_tot} eventi spia):"); soluzione_trovata = False
                        ambo_top1_tuple, count_top1 = migliori_ambi_globali_raw[0]; date_coperte_top1 = ambi_copertura_globale_eventi.get(ambo_top1_tuple, set())
                        if len(date_coperte_top1) == n_trig_tot: ambo_top1_str = format_ambo_terno(ambo_top1_tuple); messaggi_out.append(f"  - L'ambo singolo '{ambo_top1_str}' copre il 100% ({n_trig_tot}/{n_trig_tot} eventi)."); info_curr['combinazione_ottimale_copertura_100'] = {"ambi": [ambo_top1_str], "coperti": n_trig_tot, "totali": n_trig_tot, "percentuale": 100.0}; soluzione_trovata = True
                        if not soluzione_trovata and len(migliori_ambi_globali_raw) >= 2:
                            miglior_coppia_copertura = 0; miglior_coppia_ambi_str = []
                            for combo_2 in itertools.combinations(migliori_ambi_globali_raw[:min(len(migliori_ambi_globali_raw), 7)], 2):
                                amboA_tuple, _ = combo_2[0]; amboB_tuple, _ = combo_2[1]; date_A = ambi_copertura_globale_eventi.get(amboA_tuple, set()); date_B = ambi_copertura_globale_eventi.get(amboB_tuple, set()); coperte_da_coppia = date_A.union(date_B)
                                if len(coperte_da_coppia) == n_trig_tot: miglior_coppia_ambi_str = sorted([format_ambo_terno(amboA_tuple), format_ambo_terno(amboB_tuple)]); messaggi_out.append(f"  - La coppia '{miglior_coppia_ambi_str[0]}' e '{miglior_coppia_ambi_str[1]}' copre il 100% ({n_trig_tot}/{n_trig_tot} eventi)."); info_curr['combinazione_ottimale_copertura_100'] = {"ambi": miglior_coppia_ambi_str, "coperti": n_trig_tot, "totali": n_trig_tot, "percentuale": 100.0}; soluzione_trovata = True; break 
                                if len(coperte_da_coppia) > miglior_coppia_copertura: miglior_coppia_copertura = len(coperte_da_coppia); miglior_coppia_ambi_str = sorted([format_ambo_terno(amboA_tuple), format_ambo_terno(amboB_tuple)])
                            if not soluzione_trovata and miglior_coppia_ambi_str: info_curr['migliore_combinazione_parziale'] = {"ambi": miglior_coppia_ambi_str, "coperti": miglior_coppia_copertura, "totali": n_trig_tot, "percentuale": (miglior_coppia_copertura/n_trig_tot*100) if n_trig_tot > 0 else 0}
                        if not soluzione_trovata and len(migliori_ambi_globali_raw) >= 3:
                            miglior_terzina_copertura = 0; miglior_terzina_ambi_str = []
                            for combo_3 in itertools.combinations(migliori_ambi_globali_raw[:min(len(migliori_ambi_globali_raw), 7)], 3):
                                amboA_t, _ = combo_3[0]; amboB_t, _ = combo_3[1]; amboC_t, _ = combo_3[2]; dcA = ambi_copertura_globale_eventi.get(amboA_t, set()); dcB = ambi_copertura_globale_eventi.get(amboB_t, set()); dcC = ambi_copertura_globale_eventi.get(amboC_t, set()); coperte_da_terzina = dcA.union(dcB).union(dcC)
                                if len(coperte_da_terzina) == n_trig_tot: miglior_terzina_ambi_str = sorted([format_ambo_terno(amboA_t), format_ambo_terno(amboB_t), format_ambo_terno(amboC_t)]); messaggi_out.append(f"  - La terzina '{miglior_terzina_ambi_str[0]}', '{miglior_terzina_ambi_str[1]}' e '{miglior_terzina_ambi_str[2]}' copre il 100% ({n_trig_tot}/{n_trig_tot} eventi)."); info_curr['combinazione_ottimale_copertura_100'] = {"ambi": miglior_terzina_ambi_str, "coperti": n_trig_tot, "totali": n_trig_tot, "percentuale": 100.0}; soluzione_trovata = True; break 
                                if len(coperte_da_terzina) > miglior_terzina_copertura: miglior_terzina_copertura = len(coperte_da_terzina); miglior_terzina_ambi_str = sorted([format_ambo_terno(amboA_t), format_ambo_terno(amboB_t), format_ambo_terno(amboC_t)])
                            if not soluzione_trovata and miglior_terzina_ambi_str and (not info_curr.get('migliore_combinazione_parziale') or miglior_terzina_copertura > info_curr['migliore_combinazione_parziale']['coperti']): info_curr['migliore_combinazione_parziale'] = {"ambi": miglior_terzina_ambi_str, "coperti": miglior_terzina_copertura, "totali": n_trig_tot, "percentuale": (miglior_terzina_copertura/n_trig_tot*100) if n_trig_tot > 0 else 0}
                        if not soluzione_trovata:
                            ...
                            if info_curr['migliore_combinazione_parziale']: parziale = info_curr['migliore_combinazione_parziale']; messaggi_out.append(f"    La migliore copertura parziale trovata con {len(parziale['ambi'])} ambo/i ({', '.join(parziale['ambi'])}) è di {parziale['coperti']}/{parziale['totali']} eventi ({parziale['percentuale']:.1f}%).")
        ...
        if ris_graf_loc or (not all_date_trig and tipo_spia_scelto): mostra_popup_risultati_spia(info_ricerca_globale, risultati_globali)
...
def verifica_esiti_utente_su_triggers(date_triggers, combinazioni_utente, nomi_ruote_verifica, n_verifiche, start_ts, end_ts, titolo_sezione="VERIFICA MISTA SU TRIGGER"):
    ...
    # NUOVO: Struttura per memorizzare i conteggi degli sfaldamenti per item e per ruota
    ...
    for tipo_sorte_item, item_val in items_config_da_verificare:
        ...
        num_eventi_trigger_coperti_per_item = 0 # Quanti eventi spia sono stati coperti (item uscito almeno una volta)
        
        # NUOVO: Inizializza il contatore per l'item corrente
        ...
        for data_t in sorted_date_triggers:
            ...
            trigger_coperto_in_questo_ciclo_per_item = False # Flag: l'item è uscito in questo ciclo di trigger (su QUALSIASI ruota)?
            max_colpi_effettivi_per_trigger = 0
            
            # NUOVO: Set per tracciare su quali ruote l'item è già uscito per QUESTO trigger,
            # per contare ogni ruota al massimo una volta per trigger.
                ...
                if not df_fin_v.empty:
                    for colpo_idx, (_, row) in enumerate(df_fin_v.iterrows(), 1):
                        ...
                        if match: 
                            ...
                            # NUOVO: Incrementa il conteggio degli sfaldamenti per la ruota specifica
                            # se non è già stato contato un hit su questa ruota per questo evento trigger.
                            ...
                            break # L'item è uscito su questa ruota per questo trigger, passa alla prossima ruota (o al prossimo colpo se il break è interno a un altro loop)
            ...
            esiti_dettagliati_per_item[item_str_key].append(f"    Post Trg {data_t.strftime('%d/%m/%y')}: {esito_per_questo_trigger_str}")

        # Fine loop sui trigger per un item
        out.append(f"\n--- Esiti {tipo_sorte_item.upper()} ---"); 
        out.append(f"  - {item_str_key}:"); 
        out.extend(esiti_dettagliati_per_item[item_str_key])
        
        # NUOVO: Aggiungi il riepilogo degli sfaldamenti per ruota per l'item corrente
        out.append(f"\n    RIEPILOGO SFALDAMENTI SU RUOTA per {item_str_key.upper()}:")
        if not ruote_valide:
             out.append("      Nessuna ruota di verifica attiva.")
        else:
            for nome_rv_riepilogo in ruote_valide: # Itera su tutte le ruote valide per avere un output completo
        ...
        # Riepilogo generale dell'item (quanti eventi spia ha coperto)
        if num_eventi_trigger_coperti_per_item == 0: 
            out.append(f"    RIEPILOGO {item_str_key}: 0 uscite su {num_casi_trigger_base} eventi spia (0.0%)")
        else: 
            perc_copertura_item = (num_eventi_trigger_coperti_per_item / num_casi_trigger_base * 100) if num_casi_trigger_base > 0 else 0
            out.append(f"    RIEPILOGO {item_str_key}: Uscito dopo {num_eventi_trigger_coperti_per_item} su {num_casi_trigger_base} eventi spia ({perc_copertura_item:.1f}%)")

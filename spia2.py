...
# estrazioni_entry_verifica = None # Già commentato/rimosso

# MODIFICATO/NUOVO per analisi posizionale e GUI
tipo_spia_var_global = None # Tkinter StringVar, sarà inizializzato nella GUI
entry_numeri_spia = [] # Lista di widget ttk.Entry, popolata nella GUI
combo_posizione_spia = None # Tkinter ttk.Combobox, inizializzato nella GUI

# Variabili GUI per Numeri Simpatici (dalla precedente modifica)
...
def carica_dati(file_path, start_date=None, end_date=None):
    try:
        ...
        for line in lines:
            ...
            parts = line.split();
            if len(parts) < 7: continue # Data + Ruota + 5 Numeri
            data_str, ruota_str, nums_orig = parts[0], parts[1].upper(), parts[2:7]
            try:
                data_dt_val = datetime.datetime.strptime(data_str, fmt_ok)
                # Assicurati che ci siano esattamente 5 numeri e che siano interi validi
                if len(nums_orig) != 5: continue
                [int(n) for n in nums_orig] # Controlla se sono convertibili a int
            except ValueError: continue # Salta la riga se i numeri non sono validi o non 5
        ...
        # Assicura che tutte e 5 le colonne NumeroX abbiano un valore valido, altrimenti scarta la riga.
        # Utile se vuoi essere sicuro che ogni estrazione analizzata abbia 5 numeri.
...
def analizza_ruota_verifica(df_verifica, date_trigger_sorted, n_estrazioni, nome_ruota_verifica):
    ...
    for data_t in date_trigger_sorted:
        ...
        if not df_successive.empty:
            for _, row in df_successive.iterrows():
                ...
                for pos_idx, col_num_nome in enumerate(colonne_numeri):
                    ...
                    if pd.notna(num_val): # Assicura che il numero sia valido
        ...
        for ambo in ambi_unici_finestra: pres_ambi[ambo] += 1
...
def analizza_antecedenti(df_ruota, numeri_obiettivo, n_precedenti, nome_ruota):
    ...
    # Assicurati che numeri_obiettivo siano stringhe zfill(2) per il confronto
    ...
    freq_ant, pres_ant, actual_base_pres = Counter(), Counter(), 0 # Usiamo Counter per semplicità
...
# ... (aggiorna_risultati_globali, salva_risultati, format_ambo_terno INVARIATE) ...
...
# --- FUNZIONE PER POPUP DI TESTO GENERICO (NON MODALE) --- (INVARIATA)
...
# --- POPUP PER RISULTATI ANALISI SPIA --- (INVARIATA)
def mostra_popup_risultati_spia(info_ricerca, risultati_analisi):
    global root # Assicurati che root sia accessibile
    ...
    tipo_spia_usato = info_ricerca.get('tipo_spia_usato', 'N/D').upper() # Converti in maiuscolo per confronto
    spia_val = info_ricerca.get('numeri_spia_input', [])
    spia_display = ""

    if tipo_spia_usato == "ESTRATTO_POSIZIONALE": # Confronto con la stringa esatta usata
        num_spia_list = spia_val if isinstance(spia_val, list) else [spia_val] # Assicura sia lista
    ...
    elif isinstance(spia_val, tuple): # Ambo
        spia_display = "-".join(map(str, spia_val))
    elif isinstance(spia_val, list): # Estratto normale (potrebbe essere lista di uno o più numeri)
        spia_display = ", ".join(map(str, spia_val))
    else: # Fallback per valore singolo non in lista (improbabile con la logica attuale)
        spia_display = str(spia_val)

    popup_content.append(f"Tipo Spia: {tipo_spia_usato.replace('_', ' ')} ({spia_display})") # Sostituisci '_' per display
                ...
                if dati_esito:
                    ...
                    if top_pres is not None and not top_pres.empty:
                        for item, pres_val in top_pres.items():
                            ...
                            riga_base = f"      - {item_f}: {pres_val} ({perc:.1f}%) [Freq.Tot: {freq}]"
                            popup_content.append(riga_base)
                            if tipo_esito == 'estratto' and 'posizionale_presenza' in dati_esito and item_f in dati_esito['posizionale_presenza']:
                                pos_data = dati_esito['posizionale_presenza'][item_f]
                                pos_str_list = []
                                for pos_num in sorted(pos_data.keys()):
                                    pos_count = pos_data[pos_num]
                                    pos_perc = (pos_count / pres_val * 100) if pres_val > 0 else 0
                                    pos_str_list.append(f"P{pos_num}:{pos_count}({pos_perc:.0f}%)")
                                if pos_str_list:
                                    popup_content.append(f"        Posizioni (Pres.): {', '.join(pos_str_list)}")
                    else:
                        popup_content.append("      Nessuno.")
                    ...
                    if top_freq is not None and not top_freq.empty:
                        for item, freq_val in top_freq.items():
                            ...
                            riga_base = f"      - {item_f}: {freq_val} ({perc:.1f}%) [Pres. su Trigger: {pres}]"
                            popup_content.append(riga_base)
                            if tipo_esito == 'estratto' and 'posizionale_frequenza' in dati_esito and item_f in dati_esito['posizionale_frequenza']:
                                pos_data = dati_esito['posizionale_frequenza'][item_f]
                                pos_str_list = []
                                for pos_num in sorted(pos_data.keys()):
                                    pos_count = pos_data[pos_num]
                                    pos_perc = (pos_count / freq_val * 100) if freq_val > 0 else 0
                                    pos_str_list.append(f"P{pos_num}:{pos_count}({pos_perc:.0f}%)")
                                if pos_str_list:
                                    popup_content.append(f"        Posizioni (Freq.): {', '.join(pos_str_list)}")
                    else:
                        popup_content.append("      Nessuno.")
                else:
    ...
    if statistiche_combinate_dett and any(v for v in statistiche_combinate_dett.values() if v):
        popup_content.append("\n\n" + "=" * 25 + " RISULTATI COMBINATI (TUTTE LE RUOTE DI VERIFICA) " + "=" * 25)
...
def cerca_numeri(modalita="successivi"):
    ...
    if not mappa_file_ruote() or not file_ruote:
        ...
        if risultato_text:
            ...
            risultato_text.insert(tk.END, "Errore: Cartella o file non validi."); risultato_text.config(state=tk.DISABLED)
        return
    ...
    if modalita == "successivi":
        ...
        # Leggi tutti gli input numerici e valida
        numeri_spia_input_raw = [e.get().strip() for e in entry_numeri_spia]
        numeri_spia_input_validi_zfill = []
        for n_str in numeri_spia_input_raw:
            if n_str.isdigit() and 1 <= int(n_str) <= 90:
                numeri_spia_input_validi_zfill.append(str(int(n_str)).zfill(2))
            # else: Non aggiungere se non è un numero valido nel range (permette entry vuote)

        numeri_spia_da_usare = None
        spia_display_str = ""
        posizione_spia_selezionata = None

        if tipo_spia_scelto == "estratto":
            if not numeri_spia_input_validi_zfill: # Se la lista è vuota dopo la validazione
                messagebox.showwarning("Manca Input","Nessun Numero Spia (Estratto) valido inserito.")
                risultato_text.delete(1.0,tk.END); risultato_text.insert(tk.END,"Input mancante."); risultato_text.config(state=tk.DISABLED); return
            numeri_spia_da_usare = numeri_spia_input_validi_zfill # Usa tutti i numeri validi inseriti
            spia_display_str = ", ".join(numeri_spia_da_usare)

        elif tipo_spia_scelto == "estratto_posizionale":
            if not numeri_spia_input_validi_zfill or not numeri_spia_input_raw[0].strip(): # Controlla specificamente il primo entry
                messagebox.showwarning("Manca Input","Inserire il primo Numero Spia per l'analisi posizionale.")
                risultato_text.delete(1.0,tk.END); risultato_text.insert(tk.END,"Input mancante."); risultato_text.config(state=tk.DISABLED); return
            
            primo_numero_spia = numeri_spia_input_validi_zfill[0] # Deve essere il primo valido
            numeri_spia_da_usare = [primo_numero_spia]
            
            posizione_scelta_str = combo_posizione_spia.get()
            if not posizione_scelta_str or "Qualsiasi" in posizione_scelta_str:
                tipo_spia_scelto = "estratto" # Downgrade a estratto normale se Qualsiasi o non specificato
                spia_display_str = primo_numero_spia # Spia è solo il primo numero
            else:
                try:
                    posizione_spia_selezionata = int(posizione_scelta_str.split("a")[0])
                    if not (1 <= posizione_spia_selezionata <= 5): raise ValueError("Posizione fuori range")
                    spia_display_str = f"{primo_numero_spia} in {posizione_spia_selezionata}a pos."
                except ValueError as e_pos:
                    messagebox.showerror("Input Invalido",f"Posizione Spia non valida: {e_pos}")
                    risultato_text.delete(1.0,tk.END); risultato_text.insert(tk.END,"Input non valido."); risultato_text.config(state=tk.DISABLED); return
        
        elif tipo_spia_scelto == "ambo":
            if len(numeri_spia_input_validi_zfill) < 2:
                messagebox.showwarning("Manca Input","Inserire almeno 2 numeri validi per Ambo Spia.")
                risultato_text.delete(1.0,tk.END); risultato_text.insert(tk.END,"Input mancante."); risultato_text.config(state=tk.DISABLED); return
            numeri_spia_da_usare = tuple(sorted(numeri_spia_input_validi_zfill[:2]))
            spia_display_str = "-".join(numeri_spia_da_usare)
        
        else: # Dovrebbe essere impossibile arrivare qui con i radio button
            messagebox.showerror("Errore Interno", f"Tipo spia '{tipo_spia_scelto}' non gestito.")
            risultato_text.delete(1.0,tk.END); risultato_text.insert(tk.END,"Errore tipo spia."); risultato_text.config(state=tk.DISABLED); return
        ...
        info_curr={'numeri_spia_input':numeri_spia_da_usare,
                   ...
                   'start_date':start_ts,
                   'end_date':end_ts}
        if posizione_spia_selezionata is not None: # Aggiungi solo se effettivamente usata
            info_curr['posizione_spia_input'] = posizione_spia_selezionata
        ...
        for nome_ra in nomi_ra:
            ...
            # Nota: numeri_spia_da_usare è una lista per estratto/estratto_pos, tupla per ambo
            if tipo_spia_scelto == "estratto":
                dates_found_arr=df_an.loc[df_an[col_num_nomi].isin(numeri_spia_da_usare).any(axis=1),'Data'].unique()
                ...
                numero_spia_pos = numeri_spia_da_usare[0] # È una lista con un solo elemento
                colonna_target_pos = f'Numero{posizione_spia_selezionata}'
                if colonna_target_pos in df_an.columns:
                    dates_found_arr = df_an.loc[df_an[colonna_target_pos] == numero_spia_pos, 'Data'].unique()
                    dates_found_this_ruota = pd.to_datetime(dates_found_arr).tolist()
            
            elif tipo_spia_scelto == "ambo":
                spia_n1, spia_n2 = numeri_spia_da_usare[0], numeri_spia_da_usare[1]
                presente_n1 = df_an[col_num_nomi].isin([spia_n1]).any(axis=1)
                presente_n2 = df_an[col_num_nomi].isin([spia_n2]).any(axis=1)
                date_ambo_presente = df_an.loc[presente_n1 & presente_n2, 'Data'].unique()
                dates_found_this_ruota = pd.to_datetime(date_ambo_presente).tolist()
        ...
        if not all_date_trig:
            ...
            mostra_popup_risultati_spia(info_ricerca_globale, risultati_globali) # Passa info anche se vuoto
            return
        ...
        for nome_rv in nomi_rv:
            ...
            if df_ver_full is None:
                df_ver_full=carica_dati(file_ruote.get(nome_rv),start_date=start_ts, end_date=None)
                df_cache_ver[nome_rv]=df_ver_full
            ...
            for tipo_comb in ['estratto','ambo','terno']:
                ...
                if not top_punt_comb.empty:
                    ...
                    for item_idx, score_val in top_punt_comb.items():
                        ...
                        stat_dett_comb.append({
                           "item": format_ambo_terno(item_idx),
                           "punteggio": score_val,
                ...
                if not top_punt_comb.empty:
                    for i,(item_comb_idx,score_comb) in enumerate(top_punt_comb.items()):
                        item_display = format_ambo_terno(item_comb_idx)
                        messaggi_out.append(f"    {i+1}. {item_display}: Punt={score_comb:.2f} (PresAvg:{comb_perc_pres_s.get(item_comb_idx,0.0):.1f}%, FreqTot:{comb_freq_s.get(item_comb_idx,0)})\n")
...
# =============================================================================
# FUNZIONI PER VERIFICA ESITI (INVARIATE)
# =============================================================================
# ... (verifica_esiti_utente_su_triggers, verifica_esiti_futuri, esegui_verifica_futura, esegui_verifica_mista INVARIATE) ...
def verifica_esiti_utente_su_triggers(date_triggers, combinazioni_utente, nomi_ruote_verifica, n_verifiche, start_ts, end_ts, titolo_sezione="VERIFICA MISTA SU TRIGGER"):
    ...
    for nome_rv_loop in nomi_ruote_verifica:
        # Carica i dati necessari per il periodo di verifica (da start_ts in poi)
        # end_ts qui non è strettamente necessario se df_ver copre tutto il range post-trigger
    ...
    casi_tot_eff_per_ruota = len(date_triggers) 
    ...
    for trigger_idx, data_t in enumerate(date_triggers): 
        for nome_rv in ruote_valide:
            ...
            except Exception: continue # Gestisce errore se data_t non è compatibile
                ...
                for colpo_idx, (_, row) in enumerate(df_fin_v.iterrows(), 1):
                    ...
                    if estratti_u:
                        for num_hit in set_nums_draw.intersection(set(estratti_u)):
                            hits_per_item['estratto'][num_hit].append((nome_rv, colpo_idx, data_estrazione_corrente, data_t.date()))
                    if ambi_u_tpl and len(nums_draw) >= 2:
                        ...
                        for ambo_hit in ambi_generati_da_riga.intersection(set(ambi_u_tpl)):
                            hits_per_item['ambo'][ambo_hit].append((nome_rv, colpo_idx, data_estrazione_corrente, data_t.date()))
                    if terni_u_tpl and len(nums_draw) >= 3:
                        ...
                        for terno_hit in terni_generati_da_riga.intersection(set(terni_u_tpl)): 
                            hits_per_item['terno'][terno_hit].append((nome_rv, colpo_idx, data_estrazione_corrente, data_t.date()))
                    if quaterne_u_tpl and len(nums_draw) >= 4:
                        ...
                        for quaterna_hit in quaterne_generati_da_riga.intersection(set(quaterne_u_tpl)):
                            hits_per_item['quaterna'][quaterna_hit].append((nome_rv, colpo_idx, data_estrazione_corrente, data_t.date()))
                    if cinquine_u_tpl and len(nums_draw) >= 5:
                        ...
                        for cinquina_hit in cinquine_generati_da_riga.intersection(set(cinquine_u_tpl)):
                           hits_per_item['cinquina'][cinquina_hit].append((nome_rv, colpo_idx, data_estrazione_corrente, data_t.date()))
    ...
    for tipo_sorte, items_da_verificare in sorti_verificate:
        ...
        for item_utente in items_da_verificare:
            ...
            if lista_esiti_item:
                ...
                dettagli_output_per_item = []
                # Raggruppa gli esiti per data_trigger per una visualizzazione più chiara
                esiti_per_trigger_date = {}
                for ruota_h, colpo_h, data_estr_h, data_trigger_h in lista_esiti_item:
                    if data_trigger_h not in esiti_per_trigger_date:
                        esiti_per_trigger_date[data_trigger_h] = []
                    esiti_per_trigger_date[data_trigger_h].append(f"{ruota_h} @ C{colpo_h} ({data_estr_h.strftime('%d/%m')})")

                trigger_dates_sorted = sorted(esiti_per_trigger_date.keys())
                
                output_dettaglio_item = []
                for dt_trigger in trigger_dates_sorted:
                    dettagli_trigger_specifico = "; ".join(sorted(list(set(esiti_per_trigger_date[dt_trigger])))) # set per unicità
                    output_dettaglio_item.append(f"Post Trg {dt_trigger.strftime('%d/%m/%y')}: [{dettagli_trigger_specifico}]")
                
                # Limita il numero di dettagli mostrati per item per brevità
                max_dettagli_visibili = 5
                dettagli_str_completo = " | ".join(output_dettaglio_item[:max_dettagli_visibili])
                if len(output_dettaglio_item) > max_dettagli_visibili:
                    dettagli_str_completo += f" ...e altri {len(output_dettaglio_item) - max_dettagli_visibili} casi."

                out.append(f"    - {item_str}: USCITO -> {dettagli_str_completo}")
            else:
...
def verifica_esiti_futuri(top_combinati_input, nomi_ruote_verifica, data_fine_analisi, n_colpi_futuri):
    ...
    ambi_items = sorted(list(set(
        tuple(sorted(a)) for a in top_combinati_input.get('ambo', []) if isinstance(a, (list, tuple)) and len(a) == 2
    )))

    terni_items = sorted(list(set(
        tuple(sorted(t)) for t in top_combinati_input.get('terno', []) if isinstance(t, (list, tuple)) and len(t) == 3
    )))

    quaterne_items = sorted(list(set(
        tuple(sorted(q)) for q in top_combinati_input.get('quaterna', []) if isinstance(q, (list, tuple)) and len(q) == 4
    )))

    cinquine_items = sorted(list(set(
        tuple(sorted(c)) for c in top_combinati_input.get('cinquina', []) if isinstance(c, (list, tuple)) and len(c) == 5
    )))

    set_estratti = set(format_ambo_terno(e) for e in estratti_items)
    set_ambi = set(format_ambo_terno(a) for a in ambi_items)
    set_terni = set(format_ambo_terno(t) for t in terni_items)
    set_quaterne = set(format_ambo_terno(q) for q in quaterne_items)
    set_cinquine = set(format_ambo_terno(c) for c in cinquine_items)
    ...
    for nome_rv in nomi_ruote_verifica:
        ...
        if not df_fin_fut.empty:
            df_cache_ver_fut[nome_rv] = df_fin_fut
            ruote_con_dati_fut.append(nome_rv)

    if not ruote_con_dati_fut:
        return f"Nessuna estrazione trovata su nessuna ruota di verifica dopo {data_fine_analisi.strftime('%d/%m/%Y')} per {n_colpi_futuri} colpi."
        ...
        for colpo_idx, (_, row) in enumerate(df_finestra_ruota.iterrows(), 1):
            ...
            if estratti_items:
                for item_e_orig in estratti_items:
                    item_e_fmt = format_ambo_terno(item_e_orig)
                    if item_e_fmt in set_numeri_riga_formattati and hits_registrati['estratto'][item_e_orig] is None:
                        hits_registrati['estratto'][item_e_orig] = (nome_rv, colpo_idx, data_estrazione_corrente)

            if ambi_items and len(numeri_sortati_riga) >= 2:
                ambi_generati_riga = set(format_ambo_terno(a) for a in itertools.combinations(numeri_sortati_riga, 2))
                for item_a_orig in ambi_items:
                    item_a_fmt = format_ambo_terno(item_a_orig)
                    if item_a_fmt in ambi_generati_riga and hits_registrati['ambo'][item_a_orig] is None:
                        hits_registrati['ambo'][item_a_orig] = (nome_rv, colpo_idx, data_estrazione_corrente)

            if terni_items and len(numeri_sortati_riga) >= 3:
                terni_generati_riga = set(format_ambo_terno(t) for t in itertools.combinations(numeri_sortati_riga, 3))
                for item_t_orig in terni_items:
                    item_t_fmt = format_ambo_terno(item_t_orig)
                    if item_t_fmt in terni_generati_riga and hits_registrati['terno'][item_t_orig] is None:
                        hits_registrati['terno'][item_t_orig] = (nome_rv, colpo_idx, data_estrazione_corrente)

            if quaterne_items and len(numeri_sortati_riga) >= 4:
                quaterne_generati_riga = set(format_ambo_terno(q) for q in itertools.combinations(numeri_sortati_riga, 4))
                for item_q_orig in quaterne_items:
                    item_q_fmt = format_ambo_terno(item_q_orig)
                    if item_q_fmt in quaterne_generati_riga and hits_registrati['quaterna'][item_q_orig] is None:
                        hits_registrati['quaterna'][item_q_orig] = (nome_rv, colpo_idx, data_estrazione_corrente)

            if cinquine_items and len(numeri_sortati_riga) >= 5:
                cinquine_generati_riga = set(format_ambo_terno(c) for c in itertools.combinations(numeri_sortati_riga, 5))
                for item_c_orig in cinquine_items:
                    item_c_fmt = format_ambo_terno(item_c_orig)
                    if item_c_fmt in cinquine_generati_riga and hits_registrati['cinquina'][item_c_orig] is None:
                        hits_registrati['cinquina'][item_c_orig] = (nome_rv, colpo_idx, data_estrazione_corrente)
    ...
    for tipo_sorte, lista_items_sorte in sorti_config:
        if not lista_items_sorte:
            continue
        out.append(f"\n--- Esiti Futuri {tipo_sorte.upper()} ---")
        almeno_un_hit_per_sorte = False
        for item_da_verificare_orig in lista_items_sorte:
            item_str_formattato = format_ambo_terno(item_da_verificare_orig)
            dettaglio_hit = hits_registrati[tipo_sorte].get(item_da_verificare_orig)
...
def esegui_verifica_mista():
    ...
    for riga_idx, riga in enumerate(input_text.splitlines()):
        ...
        try:
            ...
            numeri_int = [int(n.strip()) for n in numeri_str if n.strip().isdigit()] # Ignora non-numeri
            ...
            # Genera solo la sorte corrispondente al numero di elementi inseriti, più le sorti inferiori
            # Es: se inserisco 1-2-3, genero il terno 1-2-3, gli ambi 1-2, 1-3, 2-3 e gli estratti 1, 2, 3.
            num_elementi = len(numeri_validi_zfill)
            if num_elementi == 1:
                combinazioni_sets['estratto'].add(numeri_validi_zfill[0])
            elif num_elementi == 2:
                combinazioni_sets['ambo'].add(tuple(numeri_validi_zfill))
                combinazioni_sets['estratto'].update(numeri_validi_zfill)
            elif num_elementi == 3:
                combinazioni_sets['terno'].add(tuple(numeri_validi_zfill))
                for ambo in itertools.combinations(numeri_validi_zfill, 2): combinazioni_sets['ambo'].add(tuple(sorted(ambo)))
                combinazioni_sets['estratto'].update(numeri_validi_zfill)
            elif num_elementi == 4:
                combinazioni_sets['quaterna'].add(tuple(numeri_validi_zfill))
                for terno in itertools.combinations(numeri_validi_zfill, 3): combinazioni_sets['terno'].add(tuple(sorted(terno)))
                for ambo in itertools.combinations(numeri_validi_zfill, 2): combinazioni_sets['ambo'].add(tuple(sorted(ambo)))
                combinazioni_sets['estratto'].update(numeri_validi_zfill)
            elif num_elementi == 5:
                combinazioni_sets['cinquina'].add(tuple(numeri_validi_zfill))
                for quaterna in itertools.combinations(numeri_validi_zfill, 4): combinazioni_sets['quaterna'].add(tuple(sorted(quaterna)))
                for terno in itertools.combinations(numeri_validi_zfill, 3): combinazioni_sets['terno'].add(tuple(sorted(terno)))
                for ambo in itertools.combinations(numeri_validi_zfill, 2): combinazioni_sets['ambo'].add(tuple(sorted(ambo)))
                combinazioni_sets['estratto'].update(numeri_validi_zfill)
    ...
    combinazioni_utente = {k: sorted(list(v)) for k, v in combinazioni_sets.items() if v} # Solo se ci sono elementi
    
    if not any(combinazioni_utente.values()): messagebox.showerror("Input Invalido","Nessuna combinazione valida estratta dagli input."); risultato_text.config(state=tk.DISABLED); return

    date_triggers = info_ricerca_globale.get('date_trigger_ordinate'); nomi_rv = info_ricerca_globale.get('ruote_verifica')
    start_ts = info_ricerca_globale.get('start_date'); end_ts = info_ricerca_globale.get('end_date') # end_ts è usato da carica_dati
    
    numeri_spia_originali = info_ricerca_globale.get('numeri_spia_input', []) 
    if isinstance(numeri_spia_originali, tuple): spia_display_originale = "-".join(map(str,numeri_spia_originali))
    elif isinstance(numeri_spia_originali, list): spia_display_originale = ", ".join(map(str,numeri_spia_originali))
    else: spia_display_originale = str(numeri_spia_originali)

    if not all([date_triggers, nomi_rv, start_ts is not None, end_ts is not None]): # start_ts e end_ts devono esistere
    ...
    try:
        ...
        lines = res_str.splitlines(); insert_idx_summary = 1 # Inserisci dopo il titolo
...
# =============================================================================
# ... (visualizza_grafici_successivi INVARIATA) ...
...
# =============================================================================
# NUOVA SEZIONE: FUNZIONI PER NUMERI SIMPATICI
# =============================================================================
def trova_abbinamenti_numero_target(numero_target_str, nomi_ruote_ricerca, start_ts, end_ts, top_n_simpatici):
    """
    Trova i numeri che sono usciti più frequentemente insieme al numero_target
    sulla stessa ruota e nella stessa estrazione.
    """
    global file_ruote # Usiamo la mappa globale dei file
    colonne_numeri = ['Numero1', 'Numero2', 'Numero3', 'Numero4', 'Numero5']

    if not numero_target_str or not (numero_target_str.isdigit() and 1 <= int(numero_target_str) <= 90):
        return None, "Numero target non valido (deve essere 1-90).", 0
    ...
    for nome_ruota in nomi_ruote_ricerca:
        if nome_ruota not in file_ruote:
            print(f"Attenzione: File per ruota {nome_ruota} non trovato in file_ruote. Sarà saltata.")
            continue
        ...
        for _, row in df_ruota.iterrows():
            # I numeri in df_ruota[colonne_numeri] sono già stringhe zfill(2) o pd.NA grazie a carica_dati
            # pd.notna filtra via i pd.NA
            ...
            if numero_target_zfill in numeri_estratti_riga:
                ...
                if altri_numeri_nella_stessa_estrazione: # Solo se ci sono altri numeri
                    abbinamenti_counter.update(altri_numeri_nella_stessa_estrazione)
    
    if not ruote_effettivamente_analizzate:
        return None, "Nessuna ruota selezionata conteneva dati validi per il periodo specificato.", 0

    if occorrenze_target == 0:
        return [], f"Numero target '{numero_target_zfill}' non trovato nelle ruote ({', '.join(ruote_effettivamente_analizzate)}) e periodo selezionati.", 0
        
    top_simpatici = abbinamenti_counter.most_common(top_n_simpatici)
    
    # Restituisce i top simpatici, nessun errore (None), e il conteggio del target
    return top_simpatici, None, occorrenze_target


def esegui_ricerca_numeri_simpatici():
    global risultato_text, root, entry_numero_target_simpatici, listbox_ruote_simpatici
    global entry_top_n_simpatici, start_date_entry, end_date_entry, cartella_entry

    # MODIFICATO: Assicura che file_ruote sia aggiornato rispetto alla cartella_entry
    if not mappa_file_ruote() or not file_ruote:
        messagebox.showerror("Errore Cartella", "Impossibile leggere i file dalla cartella specificata o la cartella non contiene file validi.\nAssicurati di aver selezionato una cartella con 'Sfoglia...'.")
        if risultato_text: # Verifica se risultato_text esiste
            risultato_text.config(state=tk.NORMAL); risultato_text.delete(1.0, tk.END)
            risultato_text.insert(tk.END, "Errore: Cartella o file non validi."); risultato_text.config(state=tk.DISABLED)
        return
    ...
    try:
        ...
        if not numero_target or not numero_target.isdigit() or not (1 <= int(numero_target) <= 90):
            ...
            if risultato_text: risultato_text.insert(tk.END, "Errore: Numero Target non valido."); risultato_text.config(state=tk.DISABLED);
            return

        selected_ruote_indices = listbox_ruote_simpatici.curselection()
        nomi_ruote_selezionate_final = []
        ...
        if not selected_ruote_indices: # Nessuna selezione utente, usa tutte le valide
            if not valid_ruote_from_listbox:
                 messagebox.showerror("Input Invalido", "Nessuna ruota valida disponibile per la ricerca.\nAssicurati di aver selezionato una cartella estrazioni con file validi.")
                 if risultato_text: risultato_text.insert(tk.END, "Errore: Ruote non valide."); risultato_text.config(state=tk.DISABLED);
                 return
            nomi_ruote_selezionate_final = valid_ruote_from_listbox
        else: # L'utente ha selezionato ruote specifiche
        ...
        if not top_n_str.isdigit() or int(top_n_str) <= 0:
            ...
            if risultato_text: risultato_text.insert(tk.END, "Errore: Top N non valido."); risultato_text.config(state=tk.DISABLED);
            return
        top_n = int(top_n_str)
        ...
        if start_dt > end_dt:
            ...
            if risultato_text: risultato_text.insert(tk.END, "Errore: Date non valide."); risultato_text.config(state=tk.DISABLED);
            return
        ...
        messagebox.showerror("Errore Input", f"Errore nella lettura degli input: {e}")
        if risultato_text: risultato_text.insert(tk.END, f"Errore input: {e}"); risultato_text.config(state=tk.DISABLED);
        return

    risultati_simpatici, errore_msg, occorrenze_target = trova_abbinamenti_numero_target(
        numero_target, nomi_ruote_selezionate_final, start_ts, end_ts, top_n
    )

    output_lines = []
    output_lines.append(f"=== Risultati Ricerca Numeri Simpatici ===")
    output_lines.append(f"Numero Target Analizzato: {numero_target.zfill(2)}")
    ruote_display_str = ', '.join(nomi_ruote_selezionate_final) if nomi_ruote_selezionate_final else "Nessuna ruota specificata o valida"
    output_lines.append(f"Ruote Analizzate: {ruote_display_str}")
    output_lines.append(f"Periodo: {start_dt.strftime('%d/%m/%Y')} - {end_dt.strftime('%d/%m/%Y')}")
    
    if errore_msg: 
        output_lines.append(f"\nMessaggio dal sistema: {errore_msg}")
    
    if risultati_simpatici is None: 
        output_lines.append("Ricerca fallita o interrotta a causa di un errore.")
    elif occorrenze_target == 0: 
        # Il messaggio da 'errore_msg' (se presente) dovrebbe già coprire questo.
        # Aggiungiamo una conferma se non c'è errore_msg esplicito.
        ...
        if not risultati_simpatici: 
            output_lines.append("Nessun altro numero abbinato trovato con frequenza significativa.")
        else:
            output_lines.append(f"\nTop {len(risultati_simpatici)} Numeri Simpatici (Numero: Frequenza Abbinamento):")
            for i, (num_simp, freq) in enumerate(risultati_simpatici):
    ...
    elif errore_msg and (risultati_simpatici is not None or occorrenze_target == 0): # Mostra popup anche per errori informativi
...
# --- INIZIO TAB NUMERI SUCCESSIVI (Spia) ---
...
controls_frame_succ.columnconfigure(2, weight=0) # Center controls
controls_frame_succ.columnconfigure(3, weight=0) # Buttons frame

# Frame per Ruote Analisi (Spia)
...
scrollbar_ra = ttk.Scrollbar(ruote_analisi_list_frame)
scrollbar_ra.pack(side=tk.RIGHT, fill=tk.Y)
...
scrollbar_ra.config(command=listbox_ruote_analisi.yview)

# Frame per Ruote Verifica (Esiti)
...
scrollbar_rv = ttk.Scrollbar(ruote_verifica_list_frame)
scrollbar_rv.pack(side=tk.RIGHT, fill=tk.Y)
...
scrollbar_rv.config(command=listbox_ruote_verifica.yview)

# Frame Centrale per Controlli Spia
...
# Inizializza tipo_spia_var_global (dovrebbe essere globale)
if 'tipo_spia_var_global' not in globals() or globals()['tipo_spia_var_global'] is None:
    tipo_spia_var_global = tk.StringVar(value="estratto")

# Funzione interna alla creazione della GUI per la tab successivi
def _toggle_posizione_spia_input_tab_succ():
    global tipo_spia_var_global, combo_posizione_spia, entry_numeri_spia
    is_posizionale = (tipo_spia_var_global.get() == "estratto_posizionale")

    if combo_posizione_spia:
        # MODIFICATO QUI: usa "readonly" quando abilitato per la modalità posizionale
        combo_posizione_spia.config(state="readonly" if is_posizionale else tk.DISABLED)
        if is_posizionale:
            current_val = combo_posizione_spia.get()
            # Se è abilitato e il valore non è valido o è vuoto, imposta il primo valore ("Qualsiasi")
            if not current_val or current_val not in combo_posizione_spia['values']:
                 combo_posizione_spia.current(0)
        else:
            combo_posizione_spia.set("Qualsiasi") # Reimposta a "Qualsiasi" quando disabilitato
...
# Frame per Numeri Spia e Posizione
spia_frame_succ = ttk.LabelFrame(center_controls_frame_succ, text=" 3. Numeri Spia / Posizione ",padding=5)
...
# Entry per i numeri spia (la variabile globale entry_numeri_spia è una lista)
entry_numeri_spia.clear() # Pulisce la lista prima di ripopolarla
for i in range(5):
...
# Combobox per la posizione (la variabile globale combo_posizione_spia)
posizioni_spia_options = ["Qualsiasi", "1a", "2a", "3a", "4a", "5a"]
if 'combo_posizione_spia' not in globals() or globals()['combo_posizione_spia'] is None:
    combo_posizione_spia = ttk.Combobox(spia_entry_container_succ, values=posizioni_spia_options, width=8, font=("Segoe UI", 9), state="disabled")
    combo_posizione_spia.set("Qualsiasi")
    combo_posizione_spia.pack(side=tk.LEFT, padx=(5,0), ipady=1)


# Frame per Estrazioni Successive
estrazioni_frame_succ = ttk.LabelFrame(center_controls_frame_succ, text=" 5. Estrazioni Successive ",padding=5)
...
# Chiamata iniziale per settare lo stato corretto del combobox
_toggle_posizione_spia_input_tab_succ()

# Frame per Pulsanti di Azione della Tab Successivi
...
# --- FINE TAB NUMERI SUCCESSIVI (Spia) ---


# --- INIZIO TAB NUMERI ANTECEDENTI ---
...
scrollbar_ra_ant = ttk.Scrollbar(ruote_analisi_ant_list_frame)
scrollbar_ra_ant.pack(side=tk.RIGHT,fill=tk.Y)
...
# Assicurati che entry_numeri_obiettivo sia una lista prima di popolarla
if 'entry_numeri_obiettivo' not in globals() or not isinstance(globals()['entry_numeri_obiettivo'], list):
    entry_numeri_obiettivo = []
else:
    entry_numeri_obiettivo.clear()
...
# --- FINE TAB NUMERI ANTECEDENTI ---


# --- INIZIO TAB NUMERI SIMPATICI ---
tab_simpatici = ttk.Frame(notebook, padding=10)
notebook.add(tab_simpatici, text=' Numeri Simpatici ')

controls_frame_simpatici_outer = ttk.Frame(tab_simpatici)
controls_frame_simpatici_outer.pack(fill=tk.X, pady=5)
controls_frame_simpatici_outer.columnconfigure(0, weight=0) # Input params
controls_frame_simpatici_outer.columnconfigure(1, weight=1) # Listbox ruote

input_params_simpatici_frame = ttk.Frame(controls_frame_simpatici_outer)
input_params_simpatici_frame.grid(row=0, column=0, sticky="ns", padx=(0,10))

lbl_numero_target = ttk.Label(input_params_simpatici_frame, text="Numero Target (1-90):", style="Title.TLabel")
lbl_numero_target.pack(anchor="w", pady=(0,2))
entry_numero_target_simpatici = ttk.Entry(input_params_simpatici_frame, width=10, justify=tk.CENTER, font=("Segoe UI",10))
entry_numero_target_simpatici.pack(anchor="w", pady=(0,10), ipady=2)
entry_numero_target_simpatici.insert(0, "10")
...
entry_top_n_simpatici = ttk.Entry(input_params_simpatici_frame, width=10, justify=tk.CENTER, font=("Segoe UI",10))
entry_top_n_simpatici.pack(anchor="w", pady=(0,10), ipady=2)
entry_top_n_simpatici.insert(0, "10")
...
ruote_simpatici_frame_outer = ttk.Frame(controls_frame_simpatici_outer)
ruote_simpatici_frame_outer.grid(row=0, column=1, sticky="nsew", padx=(10,0))
ttk.Label(ruote_simpatici_frame_outer, text="Ruote di Ricerca (CTRL/SHIFT per multiple, nessuna = tutte):", style="Title.TLabel").pack(anchor="w")
ruote_simpatici_list_frame_inner = ttk.Frame(ruote_simpatici_frame_outer)
ruote_simpatici_list_frame_inner.pack(fill=tk.BOTH, expand=True, pady=(5,0))
scrollbar_rs_y = ttk.Scrollbar(ruote_simpatici_list_frame_inner, orient=tk.VERTICAL)
scrollbar_rs_y.pack(side=tk.RIGHT, fill=tk.Y)
listbox_ruote_simpatici = tk.Listbox(
    ruote_simpatici_list_frame_inner, height=10, selectmode=tk.EXTENDED, exportselection=False,
    font=("Consolas",10), selectbackground="#005A9E", selectforeground="white",
    yscrollcommand=scrollbar_rs_y.set
)
listbox_ruote_simpatici.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
scrollbar_rs_y.config(command=listbox_ruote_simpatici.yview)
# --- FINE TAB NUMERI SIMPATICI ---


# --- CONTROLLI COMUNI INFERIORI ---
...
# --- FINE CONTROLLI COMUNI INFERIORI ---
...
def on_sfoglia_click():
    ...
    if cartella_sel:
        ...
        if mappa_file_ruote():
            ...
            listboxes_to_update = [
                listbox_ruote_analisi, listbox_ruote_verifica,
                listbox_ruote_analisi_ant, listbox_ruote_simpatici
            ]
            for lb in listboxes_to_update:
                if lb: aggiorna_lista_file_gui(lb)
        else:
            listboxes_to_reset = [
                listbox_ruote_analisi, listbox_ruote_verifica,
                listbox_ruote_analisi_ant, listbox_ruote_simpatici
            ]
            for lb in listboxes_to_reset:
                if lb:
                    lb.config(state=tk.NORMAL); lb.delete(0,tk.END)
                    lb.insert(tk.END, "Nessun file valido"); lb.config(state=tk.DISABLED)
            messagebox.showwarning("Nessun File", "Nessun file .txt valido (es. BARI.txt, ROMA.txt, ecc.) trovato nella cartella selezionata.")
btn_sfoglia.config(command=on_sfoglia_click)

def main():
    global root, risultato_text
    # Il titolo è già stato impostato alla creazione di root.
    # Le variabili globali per la GUI (tipo_spia_var_global, combo_posizione_spia, entry_numeri_spia)
    # sono manipolate/create quando la GUI stessa viene costruita.

    risultato_text.config(state=tk.NORMAL); risultato_text.delete(1.0, tk.END)
    welcome_message = (
        "Benvenuto in Numeri Spia v3.9.7!\n\n"
        ...
        "     - Tipo Spia:\n"
        "       - Estratto Spia: Inserisci 1-5 numeri.\n"
        "       - Ambo Spia: Inserisci 2 numeri.\n"
        "       - Estratto Spia Posizionale: Inserisci il 1° numero spia e scegli la sua posizione (1a-5a).\n"
        "     - Estrazioni Successive da controllare (1-18).\n"
        "     - Clicca 'Cerca Successivi'. I risultati posizionali degli esiti (estratti) saranno nel popup.\n"
        "   - 'Analisi Numeri Antecedenti (Marker)':\n"
        "     - Seleziona le Ruote da Analizzare.\n"
        "     - Inserisci i Numeri Obiettivo (marker).\n"
        "     - Indica il numero di Estrazioni Precedenti da controllare prima del marker.\n"
        "     - Clicca 'Cerca Antecedenti'.\n"
        "   - 'Numeri Simpatici':\n"
        "     - Inserisci un 'Numero Target' (1-90).\n"
        "     - Seleziona le 'Ruote di Ricerca' (se nessuna è selezionata, verranno analizzate tutte le ruote disponibili).\n"
        "     - Specifica 'Quanti Numeri Simpatici (Top N)' vuoi visualizzare.\n"
        "     - Clicca 'Cerca Numeri Simpatici' per trovare i numeri più frequentemente abbinati al target.\n"
        "\nI risultati dettagliati e i riepiloghi appariranno qui e in popup."
    )
...
# Assicurati che lo script venga eseguito solo quando chiamato direttamente

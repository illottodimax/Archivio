...
def trova_abbinamenti_numero_target(numero_target_str, nomi_ruote_ricerca, start_ts, end_ts, top_n_simpatici):
    ...
    estrazioni_con_target = [] # <-- MODIFICA: Aggiunto per memorizzare le estrazioni rilevanti
    is_online = data_source_var.get() == "Online"
    ...
    for nome_ruota in nomi_ruote_ricerca:
        ...
        for _, row in df_ruota.iterrows():
            numeri_estratti_riga = {row[col] for col in colonne_numeri if pd.notna(row[col])} # Usiamo un set per efficienza
            if numero_target_zfill in numeri_estratti_riga:
                occorrenze_target += 1
                altri_numeri_nella_stessa_estrazione = numeri_estratti_riga - {numero_target_zfill}
                if altri_numeri_nella_stessa_estrazione:
                    abbinamenti_counter.update(list(altri_numeri_nella_stessa_estrazione))
                estrazioni_con_target.append(numeri_estratti_riga) # <-- MODIFICA: Salviamo il set di numeri della riga
    ...
    # <-- MODIFICA: Restituiamo anche la lista delle estrazioni
    return abbinamenti_counter.most_common(top_n_simpatici), None, occorrenze_target, estrazioni_con_target
...
def esegui_ricerca_numeri_simpatici():
    ...
    # Preparazione dei dati per il calcolo del ritardo globale
    is_online = data_source_var.get() == "Online"
    df_completo_concatenato_simpatici = None
    dfs_to_concat_simpatici = []
    for nome_ruota in nomi_ruote_selezionate_final:
        source = URL_RUOTE.get(nome_ruota.upper()) if is_online else file_ruote.get(nome_ruota.upper())
        df_full = carica_dati(source, start_date=None, end_date=None)
        if df_full is not None:
            dfs_to_concat_simpatici.append(df_full)
    if dfs_to_concat_simpatici:
        df_completo_concatenato_simpatici = pd.concat(dfs_to_concat_simpatici).drop_duplicates().sort_values('Data')

    risultati_simpatici, errore_msg, occorrenze_target, estrazioni_con_target = trova_abbinamenti_numero_target(
    ...
    if risultati_simpatici is None: output_lines.append("Ricerca fallita o interrotta.")
        ...
        if not risultati_simpatici: output_lines.append("Nessun altro numero abbinato trovato.")
            ...
            # Calcolo dei ritardi per i numeri simpatici trovati
            ritardi_simpatici = {}
            for num_simp, _ in risultati_simpatici:
                ritardo_val = calcola_ritardo_attuale_globale(df_completo_concatenato_simpatici, str(num_simp), "estratto", end_ts)
                ritardi_simpatici[num_simp] = ritardo_val

            # Stampa dei risultati con il ritardo
            for i, (num_simp, freq) in enumerate(risultati_simpatici):
                ritardo_str = ""
                ritardo_val = ritardi_simpatici.get(num_simp, "N/D")
                if ritardo_val != "N/D":
                    ritardo_str = f" | Rit.Att: {ritardo_val}"
                output_lines.append(f"  {i+1}. Numero {str(num_simp).zfill(2)}: {freq} volte{ritardo_str}")

            # --- Blocco per Riepilogo Copertura Globale ---
            numeri_simpatici_trovati = {num for num, freq in risultati_simpatici}
            casi_coperti = 0
            for estrazione_set in estrazioni_con_target:
                altri_numeri = estrazione_set - {numero_target.zfill(2)}
                if not numeri_simpatici_trovati.isdisjoint(altri_numeri):
                    casi_coperti += 1
            
            percentuale_copertura = (casi_coperti / occorrenze_target * 100) if occorrenze_target > 0 else 0
            
            output_lines.append("\n" + "="*10 + " Riepilogo Copertura Globale " + "="*10)
            output_lines.append(f"Giocando i {len(numeri_simpatici_trovati)} numeri simpatici trovati, si sarebbe ottenuto un ambo con il numero {numero_target.zfill(2)}")
            output_lines.append(f"in {casi_coperti} occasioni su {occorrenze_target} totali ({percentuale_copertura:.1f}% di copertura).")
